...
Public Sub checkLicenceState()
    ...
    If fFExists(PzESettingsFile) Then ' does the tool's own settings.ini exist?
        slicence = fGetINISetting("Software\PzEarth", "Licence", PzESettingsFile)
...
Public Sub setMainTooltips()
   On Error GoTo setMainTooltips_Error
    ...
    If PzEEnableTooltips = "1" Then
        'globeWidget.Widget.FontName = PzEPrefsFont ' does not apply to the tooltip
...
Public Sub makeVisibleFormElements()
    ...
    fMain.globeForm.Left = Val(fGetINISetting("Software\PzEarth", "maximiseFormX", PzESettingsFile)) ' / screenPixelsPerPixelX
    fMain.globeForm.Top = Val(fGetINISetting("Software\PzEarth", "maximiseFormY", PzESettingsFile)) ' / screenPixelsPerPixelY
   ...
   On Error GoTo mainScreen_Error
    ...
    If aspectRatio = "landscape" Then
        If PzEWidgetLandscape = "1" Then
            If PzELandscapeFormHoffset <> vbNullString Then
                fMain.globeForm.Left = Val(PzELandscapeFormHoffset)
                fMain.globeForm.Top = Val(PzELandscapeFormVoffset)
            End If
        End If
    ...
    If aspectRatio = "portrait" Then
        If PzEWidgetPortrait = "1" Then
            fMain.globeForm.Left = Val(PzEPortraitHoffset)
            fMain.globeForm.Top = Val(PzEPortraitYoffset)
        End If
    ...
    If PzEWidgetPosition = "1" Then
        PzEhLocationPercPrefValue = Str$(fMain.globeForm.Left / screenWidthPixels * 100)
        PzEvLocationPercPrefValue = Str$(fMain.globeForm.Top / screenHeightPixels * 100)
    End If
...
Public Sub globeForm_Unload() ' name follows VB6 standard naming convention
    ...
    PzEMaximiseFormX = Str$(fMain.globeForm.Left) ' saving in pixels
    PzEMaximiseFormY = Str$(fMain.globeForm.Top)
...
Public Sub readPrefsPosition()
   ...
   On Error GoTo readPrefsPosition_Error
    ...
    PzEFormXPosTwips = fGetINISetting("Software\PzEarth", "formXPos", PzESettingsFile)
    PzEFormYPosTwips = fGetINISetting("Software\PzEarth", "formYPos", PzESettingsFile)

    ' if a current location not stored then position to the middle of the screen
    If PzEFormXPosTwips <> "" Then
        panzerEarthPrefs.Left = Val(PzEFormXPosTwips)
    Else
        panzerEarthPrefs.Left = screenWidthTwips / 2 - panzerEarthPrefs.Width / 2
    End If

    If PzEFormYPosTwips <> "" Then
        panzerEarthPrefs.Top = Val(PzEFormYPosTwips)
    Else
...
Public Sub writePrefsPosition()
   ...
   On Error GoTo writePrefsPosition_Error
    ...
    If panzerEarthPrefs.WindowState = vbNormal Then ' when vbMinimised the value = -48000  !
        ...
        sPutINISetting "Software\PzEarth", "formYPos", PzEFormYPosTwips, PzESettingsFile
    End If
